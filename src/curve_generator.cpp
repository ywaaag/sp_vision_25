...
auto build_reference(double elapsed_s, double amplitude_rad, double frequency_hz)
{
  auto yaw0 = evaluate_target_yaw(elapsed_s, amplitude_rad, frequency_hz);
  ...
  for (int i = 0; i < auto_aim::HORIZON; ++i) {
    ...
    auto yaw = evaluate_target_yaw(sample_t, amplitude_rad, frequency_hz);
    auto yaw_prev = evaluate_target_yaw(sample_t - auto_aim::DT, amplitude_rad, frequency_hz);
    auto yaw_next = evaluate_target_yaw(sample_t + auto_aim::DT, amplitude_rad, frequency_hz);
    auto yaw_vel = tools::limit_rad(yaw_next - yaw_prev) / (2.0 * auto_aim::DT);

    traj.col(i) << tools::limit_rad(yaw - yaw0), yaw_vel, 0.0, 0.0;
  }
...
int main(int argc, char * argv[])
{
  ...
  auto report_fire_ratio = [&](const std::chrono::steady_clock::time_point & now) {
    ...
    if (sample_count == 0) {
      tools::logger()->info(
        "[curve_generator] last {:.1f}s no AUTO_AIM samples, amplitude {:.2f} deg, frequency {:.2f} Hz",
        REPORT_INTERVAL_S, amplitude_deg, frequency_hz);
    } else {
      const auto fire_ratio = static_cast<double>(fire_count) / static_cast<double>(sample_count);
      tools::logger()->info(
        "[curve_generator] last {:.1f}s theoretical fire ratio {:.2f}% ({}/{}), amplitude {:.2f} deg, frequency {:.2f} Hz",
        REPORT_INTERVAL_S, fire_ratio * 100.0, fire_count, sample_count, amplitude_deg, frequency_hz);
    }
  ...
  while (!exiter.exit()) {
    ...
    auto [yaw0, traj] = build_reference(elapsed_s, amplitude_rad, frequency_hz);
    ...
    nlohmann::json data;
    data["t"] = elapsed_s;
    data["curve_amplitude_deg"] = amplitude_deg;
    data["curve_frequency_hz"] = frequency_hz;
