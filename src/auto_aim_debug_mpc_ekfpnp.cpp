...
#include "tasks/auto_aim_ekfpnp/tracker.hpp"
...
int main(int argc, char * argv[])
{
  ...
  auto_aim_ekfpnp::Tracker tracker(config_path, solver);
  auto_aim::Planner planner(config_path);
  AutoAimDebugDashboard dashboard(dashboard_config, config_path, planner);

  tools::ThreadSafeQueue<std::optional<auto_aim_ekfpnp::Target>, true> target_queue(1);
  ...
  auto plan_thread = std::thread([&]() {
    ...
    while (!quit) {
      std::optional<auto_aim_ekfpnp::Target> target;
  ...
  while (!exiter.exit()) {
    ...
    tools::draw_text(
      img, fmt::format("imu delay {:.2f} ms | ekfpnp", imu_delay_ms), cv::Point(30, 40));
    cv::resize(img, img, {}, 0.5, 0.5);
    cv::imshow("reprojection_ekfpnp", img);
